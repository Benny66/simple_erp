  ...
  <el-table ref="multipleTable" :data="formatData" :row-style="showRow" v-bind="$attrs">   <!--  @header-click="chooseall" -->
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        ...
        <el-button type="danger" icon="el-icon-delete" circle @click="deleteCategoryInfo(scope.row.id)"></el-button>
      </template>
      <template>
...
export default {
  ...
  methods: {
    ...
    editCategoryInfo: function (row) {
      this.$emit(this.handleFunc.edit, row)
    },
    addCategoryInfo: function (row) {
      this.$emit(this.handleFunc.add, row)
    },
    deleteCategoryInfo: function (row) {
      this.$emit(this.handleFunc.delete, row)
    },
  },
  mounted() {
    this.$nextTick(() => {
